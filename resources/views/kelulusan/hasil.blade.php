    ...
    <div class="card">
      ...
      <div class="c-body">
        ...
        <div class="grid">
          ...
          <div class="gi"><span style="color:var(--muted); font-size: 0.72rem; text-transform:uppercase; letter-spacing: 0.05em; display:block; margin-bottom: 4px;">Jenis Kelamin</span>{{ $siswa->jenis_kelamin ? ucwords(str_replace('-', ' ', $siswa->jenis_kelamin)) : '-' }}</div>
          <div class="gi"><span style="color:var(--muted); font-size: 0.72rem; text-transform:uppercase; letter-spacing: 0.05em; display:block; margin-bottom: 4px;">Tempat, Tanggal Lahir</span>{{ $siswa->tempat_lahir ?? '-' }}, {{ $siswa->tanggal_lahir ? \Carbon\Carbon::parse($siswa->tanggal_lahir)->locale('id')->translatedFormat('d F Y') : '-' }}</div>
        </div>
