...
@section('navbar-title', 'Edit Role')
  ...
  <div class="col-12">
    <div class="d-flex align-items-center justify-content-between mb-4">
      <div>
        ...
        <p class="text-muted mb-0">Ubah konfigurasi role dan hak aksesnya.</p>
      </div>
    ...
    <div class="panel shadow-sm">
      <div class="panel-body p-4">
        <form action="{{ route('admin.role-management.update', $role->id) }}" method="POST">
          @csrf
          @method('PUT')
          <div class="mb-4">
            ...
            <div class="input-group input-group-merge">
              ...
              <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="Contoh: editor, supervisor" value="{{ old('name', $role->name) }}" required {{ in_array($role->name, ['admin', 'operator']) ? 'readonly' : '' }} />
            </div>
            @error('name')
              <div class="invalid-feedback d-block">{{ $message }}</div>
            @enderror
            @if(in_array($role->name, ['admin', 'operator']))
              <div class="form-text text-warning"><i class="ti tabler-alert-triangle me-1"></i> Nama role '{{ $role->name }}' tidak dapat diubah karena merupakan sistem inti.</div>
            @else
                ...
                <div class="col-md-4">
                  <div class="form-check custom-option custom-option-basic">
                    <label class="form-check-label custom-option-content" for="permission_{{ $permission->id }}">
                      <input class="form-check-input" type="checkbox" name="permissions[]" value="{{ $permission->name }}" id="permission_{{ $permission->id }}" {{ in_array($permission->name, $rolePermissions) ? 'checked' : '' }}>
