...
<head>
  ...
  <title>Legalisir Ijazah — MAN 1 Kota Bandung</title>
...
<body>
  ...
  <div class="form-container">
    <div class="form-header">
      <h1>Legalisir Ijazah</h1>
      <p>Layanan legalisir ijazah untuk alumni MAN 1 Kota Bandung</p>
    </div>

    <div class="info-banner">
      <i class="ti ti-info-circle"></i>
      <p>
        <strong>Untuk Alumni</strong> — Layanan ini khusus bagi alumni yang membutuhkan legalisir fotokopi ijazah.
        Pastikan data yang diisi sesuai dengan identitas saat masih menjadi siswa.
      </p>
    </div>

    <form id="legalisirForm">
      <div class="row">
      ...
      <div class="row">
        <div class="form-group">
          <div class="input-wrapper">
            <i class="ti ti-file-certificate input-icon"></i>
            <input type="number" name="jumlah_lembar" class="form-control" placeholder="Jumlah Lembar" min="1" max="50" required>
            <div class="invalid-feedback" data-field="jumlah_lembar"></div>
          </div>
        </div>
        <div class="form-group">
          <div class="input-wrapper">
            <i class="ti ti-brand-whatsapp input-icon"></i>
            <input type="text" name="no_wa" class="form-control" placeholder="Nomor WhatsApp Aktif" required>
      ...
      <div class="form-group" style="grid-column: 1 / -1;">
        <div class="input-wrapper">
          <i class="ti ti-notes input-icon" style="top:20px;transform:none;"></i>
          <textarea name="keperluan" class="form-textarea" placeholder="Keperluan legalisir (opsional) — contoh: untuk daftar kuliah, melamar pekerjaan, dll."></textarea>
          <div class="invalid-feedback" data-field="keperluan"></div>
        </div>
      </div>

      <button type="submit" class="btn-submit" id="btnSubmit">
        <i class="ti ti-paper-plane"></i> Ajukan Legalisir Ijazah
      </button>
    </form>
  </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      const form = document.getElementById('legalisirForm');
      ...
      form.addEventListener('submit', async function(e) {
        ...
        try {
          const response = await fetch('{{ route("ptsp.legalisir-ijazah.store") }}', {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            Swal.fire({
              icon: 'success',
              title: 'Berhasil!',
              html: 'Permohonan legalisir ijazah berhasil dikirim.<br><br><strong>Nomor Tiket:</strong><br><span style="font-size:1.5rem;font-weight:800;color:#34d399;letter-spacing:2px">' + data.no_tiket + '</span>',
          ...
          btnSubmit.innerHTML = '<i class="ti ti-paper-plane"></i> Ajukan Legalisir Ijazah';
        }
      });
    });
  </script>
